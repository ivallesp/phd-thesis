...
\begin{figure}[ht]
	...
	\caption[Speech commands precision-recall for \textit{35-words-recognition} task]{Precision and recall for each of the different classes using the \textit{35-words-recognition} model trained with data version V2. Classes are sorted by descending f1-score.}
...
\begin{table}[h!]
	\centering
	\caption[Speech commands recognition accuracy for dataset v1]{Accuracy (mean $\pm$ standard deviation) obtained by the proposed solution on the different tasks compared to other benchmark algorithms (described in \autocite{Andrade2018, Mcmahan2017, Warden2018,Zhang2017}) and compared to the measurement of human accuracy (through 4 manual evaluations). The results of best performing algorithms for each task have been highlighted in bold in each case. Results better than human performance (given a statistical significance level of $\alpha = 0.05$) have been tagged with a star mark (*).}
